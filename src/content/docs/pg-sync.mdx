...
|------|---------|-------------|
| `--full` | `false` | Force full resync and push, bypassing change detection |
...
Incremental pushes use a content heuristic (message count and
content length statistics) to skip sessions that haven't
changed. Use `--full` to bypass this and re-push everything —
for example, after a schema reset or when session content was
rewritten in place.
...
| `-public-port` | | Public-facing port |
...
| `machine_name` | system hostname | Identifies the pushing machine |
