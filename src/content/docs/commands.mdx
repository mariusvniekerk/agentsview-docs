...
Print machine-readable token usage data for scripting and
...
Use this command when you want token usage in a structured format
instead of the in-app session header display. It is intended for
shell scripts, CI jobs, and other tooling that needs to consume
token totals programmatically.
