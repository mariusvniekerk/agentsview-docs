...
- **Agent** — select one or more agents (Claude, Copilot, Codex,
  ...
  iFlow, Zencoder) to filter sessions. Click an agent to toggle its
  selection; multiple agents can be active at once.
...
Right-click a session in the sidebar and select **Delete** to
move it to the trash. An undo toast appears briefly to let you
recover the session immediately. Trashed sessions are hidden
from all listings and analytics.
...
Changes are persisted to `~/.agentsview/config.json` and
