...
Subagent sessions spawned by the Task tool are hidden from the
top-level session list and are accessible only through their
parent session's tool blocks (see
