...
On the normal local `agentsview serve` runtime, the dashboard and
the Usage page also refresh automatically when new sync data lands.
AgentsView listens to a global SSE event stream and refetches the
affected data instead of forcing you to reload the page manually.
...
![Session Health section on the dashboard](/screenshots/session-health.png)
...
As of 0.23.0, those active filters are also preserved when you
switch between the **Sessions** and **Usage** tabs, so you can move
between browsing and cost analysis without rebuilding the same view.
...
includes styled message rendering and works offline.
...
export. The markdown export is particularly well-suited for handing
session context to another agent.
